...
public class PDF {
    ...
    public static boolean exportBillsPDF(List<Bill> bills, Day from, Day to, String path) {
        ...
        for (Bill bill : bills) {
            ...
            data[1] = bill.getStaffID();
            data[2] = bill.getDateOfPurchase().toMySQLString();
            data[3] = VNString.currency(bill.getTotal());
            data[4] = VNString.currency(bill.getReceived());
            data[5] = VNString.currency(bill.getExcess());
            tableData.add(data);
        }
        pdf.addTable(tableData, 16F, 4.3F, 5F, new float[]{0.5F, 1.2F, 2.5F, 3.2F, 4.5F, 5.5F, 6.5F});
        pdf.closeDocument(file);
        return true;
    }
